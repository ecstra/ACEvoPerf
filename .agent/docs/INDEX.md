...
- [session-leak-fix](systems/session-leak-fix.md), the switch that frees the practice and menu sessions the game keeps in memory, the cycle between a local server connection and its game mode it breaks, the one call it patches and what it has run on
- [responsive-ui](systems/responsive-ui.md), the one switch that keeps the menus smooth and the HUD's driving frame times even, its seven parts, what each patches in Cohtml, the exe or the pages and how it checks the build, and the shared Cohtml hooks
