...
| `tile_pool_mb` | 0 | tile pool size in MB, honoured with or without the canonical flag (`0x1C80EC4` returns before the flag is read), created once at start | `auto` by default, 512, 1024, 1536, 2048 or 3072 by the card's memory and left to the game under 2 GB, DEC-005 and DEC-009 |
