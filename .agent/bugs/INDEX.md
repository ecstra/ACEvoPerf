...
- [BUG-009-one-percent-lows-far-below-average](BUG-009-one-percent-lows-far-below-average.md), the same gap on the owner's 5070 desktop with no integrated GPU rules out the integrated GPU present path, left are spread out renderer code, a ripple from one UI view updated per frame and frames held for a refresh the frame rate is above, the UI schedule test and a 60 Hz stint next in one launch
