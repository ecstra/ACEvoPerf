...
- [BUG-029-the-hud-restyles-most-of-its-page-while-driving](BUG-029-the-hud-restyles-most-of-its-page-while-driving.md), now and then the HUD restyles a large part of its page on the UI worker, about 24 ms of Cohtml style matching, and the game thread waits for it at EvoUi's end of frame, one 35 ms frame and a 30 fps reading on a short window overlay, a child list change on the HUD's top level from Cohtml itself, named for the wrong way label's data-bind-if, fixed candidate in the responsive UI's child removal fix
