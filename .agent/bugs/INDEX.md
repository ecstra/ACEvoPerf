...
- [BUG-009-one-percent-lows-far-below-average](BUG-009-one-percent-lows-far-below-average.md), the same gap on the owner's 5070 desktop with no integrated GPU rules out the integrated GPU present path, left are spread out renderer code and a ripple from one UI view updated per frame, the UI schedule test next
