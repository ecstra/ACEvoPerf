...
- `memory_census` under `[developer]` writes `acevo_perf_memory.csv` every ten seconds, what the
  game's committed memory is made of, how much each heap holds and which code committed the blocks
  still in use, to track down the memory the game keeps growing across scene loads. Diagnostics,
  off by default.
