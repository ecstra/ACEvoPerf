...
static DWORD WINAPI TimelineThread(void*)
{
    ...
    HANDLE framesCsv = g_cfg.frames ? OpenCsv(L"acevo_perf_frames.csv", "t_s,frame_ms,present_ms,wait_ms,fence_ms,tile_req,f2m_req,gpumem_req\r\n") : INVALID_HANDLE_VALUE;
    ...
    for (;;) {
        ...
        char tmp[80];
        for (auto& fr : buf) {
            int m = _snprintf_s(tmp, sizeof tmp, _TRUNCATE, "%.3f,%.2f,%.2f,%.2f,%.2f,%u,%u,%u\r\n", fr.t, fr.ms, fr.present, fr.wait, fr.fence, fr.tiles, fr.f2m, fr.gpumem);
            out.append(tmp, m);
        }
