...
static DWORD WINAPI TimelineThread(void*)
{
    ...
    for (;;) {
        ...
        char tmp[80];
        for (auto& fr : buf) {
            int m = _snprintf_s(tmp, sizeof tmp, _TRUNCATE, "%.3f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%u,%u,%u,%u\r\n", fr.t, fr.ms, fr.present, fr.wait, fr.fence, fr.tileMap, fr.execute, fr.mappedTiles, fr.tiles, fr.f2m, fr.gpumem);
            out.append(tmp, m);
        }
